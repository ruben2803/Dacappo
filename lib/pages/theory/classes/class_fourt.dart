...
class ClassFourt extends StatefulWidget {
  const ClassFourt({Key? key}) : super(key: key);

  @override
  State<ClassFourt> createState() => _ClassFourtState();
}

class _ClassFourtState extends State<ClassFourt> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Clase 4 - ¿Qué es la clave de sol?'),
        centerTitle: true,
      ),
      body: SingleChildScrollView(
        child: Padding(
          ...
          child: Column(
            children: [
              const Text(
                'Clase 4 - ¿Qué es la clave de sol?',
                style: TextStyle(
                  fontSize: 18,
              ...
              FloatingActionButton(
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => const ExampleNotesG1(),
                    ),
                  );
                },
